...
gg :: Eq a => [a] -> [[a]]
gg [] = []
gg (h:ts) = let (l1,l2) = ggAux h (h:ts) in l1 : gg l2 where
	ggAux x [] = ([],[])
	ggAux x (h:ts) = if (x == h) then (h : (fst (ggAux x ts)), snd (ggAux x ts)) else ([], (h:ts))
...
nub' list = nubAux list [] where
	nubAux [] new = new
	nubAux (x:xs) new = if (elem x new == False) then nubAux xs (new ++ [x]) else nubAux xs new
...
-- 		posImparesAux _ [] = []
-- 		posImparesAux i (h:ts) = if (mod i 2 == 0) then posImparesAux (i+1) ts else h : (posImparesAux (i+1) ts)
...
-- 		posParesAux _ [] = []
-- 		posParesAux i (h:ts) = if (mod i 2 == 0) then h : (posParesAux (i+1) ts) else posParesAux (i+1) ts
